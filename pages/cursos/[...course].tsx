...
const Curso = ({ course, topic }: any) => {
    ...
    const { slug, title, description, topics, tags } = course;
    ...
    return topic ? (
        <div className={style.course__page}>
            <div className={style.header__buttons}>
                <div className={style.back__button} onClick={() => { history.go(-2) }}>
            ...
            <div className={style.course}>
                ...
                <div className={style.course__content}>
...

                        {
                            topicVideo ? (
                                <div className={style.video}>
                                    <iframe id="course__video__player" width="100%" height="100%" typeof="text/html"
                    ...
                    <div className={`${style.course__topics} ${showTopics ? `${style.show}` : ''}`}>
                        <div className={style.header}>
                            <h6>Contenido</h6>
                            <div className={style.close__button} onClick={() => setShowTopics(!showTopics)}>
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    height="42px"
                                    width="42px"
                                    viewBox="0 0 24 24">
                        ...
                        <div className={style.topics}>
                            {
                                topics.map((topic: any, index: number) => (
                                    <div key={index}
                                        className={`${style.topic} ${topic.slug === tema ? style.activated : ''}`}
                                        onClick={() => setShowTopics(!showTopics)}>
                                        <Link href={`/cursos/${slug}/${topic.slug}`} className={style.topic__ancor}>
        ...
        <div className={style.course__page}>
            <div className={style.header__buttons}>
                <div className={style.back__button} onClick={() => { history.back() }}>
                    <svg xmlns="http://www.w3.org/2000/svg"
                        height="30px" viewBox="0 0 24 24"
                        width="30px">
                        <path d="M0 0h24v24H0V0z" fill="none" />
            ...
            <div className={style.course}>
                ...
                <div className={style.initial__content}>
                    ...
                    <MdEditor
                        view={{ menu: false, md: false, html: true }}
                        renderHTML={() => mdParser.render(description)} />


                    {

                        topics[0]?.slug ? (
                            <Link href={`/cursos/${slug}/${topics[0].slug}`} className={style.start__button}>
                                Comenzar el curso
                            </Link>
                        ) : (
